...
function getById (req, res) {
  return Orders.findById({ _id: req.params.id })
    .then(userList => {
      Items.populate(userList, { path: "itemsorder.item", model: 'Item', select: '-image'}).
      then(listItems => {
...
function getByUserId (req, res) {
  return Orders.find({ user: req.params.id }, {}, { sort:{ orderdate: -1 } })
    .then(userList => {
      Items.populate(userList, { path: "itemsorder.item", model: 'Item',  select: '-image' }).
      then(listItems => {
        Clients.populate(listItems, { path: "client", model: 'Client' })
...
function getByClientId (req, res) {
  return Orders.find({ client: req.params.id }, {}, { sort:{ orderdate: -1 } })
    .then(userList => {
      Items.populate(userList, { path: "itemsorder.item", model: 'Item' , select: '-image'}).
      then(listItems => {
        Clients.populate(listItems, { path: "client" })
...
function update (req, res) {
  ...
  return Orders.findOneAndUpdate({ _id: req.params.id }, { $set: newOrder }, { new: true })
    .then(item => {
...
function updateStatus (req, res) {
  ...
  return Orders.findOneAndUpdate({ _id: req.params.id }, { $set: { status } }, { new: true })
    .then(item => {
      Users.populate(item, {path: "user", model: 'User'})
        .then(listItems => {
          Clients.populate(listItems, { path: "client" })
            .then(itemClient => {
              var orderStatus = statusListOrder.find(itemStatus => itemStatus.id === itemClient.status).text;
              var customMessage = 'Su Pedido para '+ itemClient.client.fullname + ' fue ' + orderStatus;
              var message = {
                app_id: ONESIGNAL_APP_ID,
                contents: {"en": customMessage},
...
function getBasicReport (req, res) {
  ...
  return Orders.find({ deliverydate: {
